...
from flocks.session.prompt import SystemPrompt, SessionPrompt, load_prompt_fragment
...
Please address this message and continue with your tasks.
    ...
    async def _build_channel_context_prompt(self) -> Optional[str]:
        ...
        try:
            ...
            if not bindings:
                return load_prompt_fragment(
                    "runtime/channel_context_web.md",
                    session_id=self.session.id,
                )
    ...
    def _get_tool_instructions(self) -> str:
        ...
        if self._should_use_text_tool_call_mode():
            return load_prompt_fragment("runtime/minimax_tool_instructions.md")
    ...
    def _build_tool_catalog_prompt(self, agent: AgentInfo) -> Optional[str]:
        ...
        is_rex = getattr(agent, "name", "") == "rex"
        template = (
            "runtime/tool_catalog_awareness_rex.md"
            if is_rex
            else "runtime/tool_catalog_awareness_agent.md"
        )
        return load_prompt_fragment(
            template,
    ...
    def _build_text_tool_call_catalog_prompt(self, tools: List[Dict[str, Any]]) -> Optional[str]:
        ...
        lines = load_prompt_fragment("runtime/text_tool_call_catalog_header.md").splitlines()
        for tool in tools:
    ...
    async def _to_chat_messages(
        ...
        await self._write_model_context_debug_log(chat_messages)
        
        return chat_messages

    async def _write_model_context_debug_log(self, chat_messages: List[ChatMessage]) -> None:
        try:
            from flocks.config import Config
            from flocks.workspace.manager import WorkspaceManager

            cfg = await Config.get()
            if not (cfg.debug and cfg.debug.model_context):
                return
            ...
            log_path = output_dir / "Context.log"
            payload = {
            ...
            log.debug("runner.context_debug_log_failed", {"error": str(exc)})
